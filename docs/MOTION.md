...
- Depth movement is bounded by each element's declared intensity. Preference changes and cleanup remove stale inline values. Unused scene and pointer-tilt handlers have been removed.
...
A leaving-phase timer releases the navigation overlay even if the browser omits `transitionend`, including when reduced motion is enabled during navigation.
